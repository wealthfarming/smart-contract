...
contract WealthFarming is AccessControl, Pausable, ReentrancyGuard{
    ...
    */
    ...
    /**
     * @dev For revenue fee, 4000 = 40%
     */
    uint256 public baseRevenue = 4000;
     ...
     * @dev Address of receive fee trade
     */
    address public receiveProfitShare;


    /**
     * @dev Address of receive fee trade
     */
    address public receiveFund;
   ...
   * @dev totalSellInSession
    ...
    * @dev nft price
    */
    mapping(uint256 => uint256) mapNFTIdPrice;
    ...
    constructor(address _receiveFund, address _receiveFee, address _receiveProfitShare) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);

        receiveFee = _receiveFee;
        receiveFund = _receiveFund;
        receiveProfitShare = _receiveProfitShare;
    ...
    function calculateNFTPrice(uint256 _totalAsset, uint256 _winRate, uint256 _risk) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        ...
        // Calculate nft price
        uint256 currentPrice = _totalAsset  / totalNFT;
        // Calculate number of mint NFT
    ...
    function finalizeBuyNFT(uint256 _transactionId) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        ...
        while (lockNFT.length > 0 && nftPrice <= baseAmount) {
            ...
            // get tokenId of NFT from locked array
            ...
            // update nft price
            mapNFTIdPrice[tokenId] = nftPrice;
        ...
        while(nftPrice <= baseAmount) {
            ...
            // update nft price
            mapNFTIdPrice[nftToken.getNextTokenId() - 1] = nftPrice;
        ...
        // Send fee to fund wallet
        usdcToken.transfer(receiveFund, txn.amount - totalFeeValue - baseAmount);
    ...
    function finalizeSellNFT(uint256 _sellId) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        ...
        uint256 lastNFTPrice = mapNFTIdPrice[txn.tokenId];

        uint256 totalFee = fee;

        // Update fee
        if (lastNFTPrice > txn.price) {
            uint256 revenueFee = (lastNFTPrice - txn.price) * baseRevenue / FEE_FACTOR;
            totalFee += revenueFee;
            mapNFTIdPrice[txn.tokenId] = txn.price;

            if (revenueFee > 0) {
                // send fee to admin
                usdcToken.transfer(receiveProfitShare, revenueFee);
            }
        }

        // refund usdc to buyer
        usdcToken.transfer(txn.seller, txn.price - totalFee);
    ...
    * @dev Allow contract for interact with this market
    ...
    /**
     * @dev Update receive fee
     * @param newReceiveFee is new receive fee address
     */
    function updateReceiveFee(address newReceiveFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        receiveFee = newReceiveFee;
    }

    /**
     * @dev Update receive fee
     * @param newReceiveFund is new receive fee address
     */
    function updateReceiveFund(address newReceiveFund) external onlyRole(DEFAULT_ADMIN_ROLE) {
        receiveFund = newReceiveFund;
    }

    /**
     * @dev Update receive fee
     * @param newReceive is new receive fee address
     */
    function updateReceiveProfitShare(address newReceive) external onlyRole(DEFAULT_ADMIN_ROLE) {
        receiveProfitShare = newReceive;
    }

    /**
     * @dev Update base fee
     * @param newBaseFee is new base fee
     */
    function updateBaseFee(uint256 newBaseFee) external  onlyRole(DEFAULT_ADMIN_ROLE) {
        baseFee = newBaseFee;
    }

    /**
     * @dev Update base fee
     * @param newbaseRevenue is new base fee
     */
    function updateBaseRevenue(uint256 newbaseRevenue) external  onlyRole(DEFAULT_ADMIN_ROLE) {
